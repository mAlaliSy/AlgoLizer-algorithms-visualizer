...
class GreedyBestFirstAlgorithmRunner(grid: Array<Array<TileType>>) : ShortestPathAlgorithmRunner(grid) {
        ...
        while (queue.isNotEmpty()) {
            ...
            for (neighbor in findNeighbors(node.position)) {
                ...
                if (ncost < costs[neighbor.first][neighbor.second]) {
                    ...
                    queue.add(ShortestPathNode(neighbor, heuristic(neighbor, destination), node))
